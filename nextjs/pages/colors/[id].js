      ...
      <div>
        {
          isLoading
            ...
            <>
              ...
              <p className={customStyles.p2}>{currentColor.description}.  <a href={currentColor.uri}>Click here to see this NFT on opensea.io.</a>
              </p>
              <div className={customStyles.c1}>{currentColor.name}</div>
              <div className={customStyles.carousel}>
                <Carousel {...getConfigurableProps()} onChange={(index) => handleClick(index)}>
                  {
                    Object.keys(colors).map((color, index) => {
                      let data = colors[color];
                      let url = "/colors/" + data.name.substring(1);
                      return (
                        <div key={index} onClick={() => router.push({ pathname: url }, undefined, { scroll: false })}>
